...
const Server = async () => {
  ...
  // Load SSL certificate and private key (replace with your own paths)
  ...
  await server.start();
  const corsOptions = {
    origin: ["http://localhost:3000", "http://13.203.157.245:3000"],
  };
  // app.use(cors());
  ...
  // Cron job to toggle coupon code status every minute (* * * * *), every hour (0 * * * *) and every day (0 0 * * *)
  ...
  console.log(
    `🚀 Server ready at https://localhost:${process.env.PORT}/api/graphql`
  );
};
